...
func GetNodeConditionPanel(cmd *cobra.Command, clientAuth *model.Auth) (map[string]float64, *NodeConditionPanel, error) {
    ...
    if diskPressureAvg != 0 {
        pressureValues["disk_pressure"] = diskPressureAvg
    }
    if memoryPressureAvg != 0 {
        pressureValues["memory_pressure"] = memoryPressureAvg
    }
    if pidPressureAvg != 0 {
        pressureValues["pid_pressure"] = pidPressureAvg
    }

    return pressureValues, nodeConditionPanel, nil
}
