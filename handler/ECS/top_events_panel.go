...
func GetECSTopEventsData(cmd *cobra.Command, clientAuth *model.Auth, cloudWatchLogs *cloudwatchlogs.CloudWatchLogs) ([]*cloudwatchlogs.GetQueryResultsOutput, error) {
	...
	results, err := commanFunction.GetLogsData(clientAuth, startTime, endTime, logGroupName, `fields @timestamp, @message| filter eventSource = 'ecs.amazonaws.com'| stats count() as count by eventName, @timestamp| limit 10`, cloudWatchLogs)
	if err != nil {
		return nil, nil
	}
