...
func GetECSResourceUpdatedEvents(cmd *cobra.Command, clientAuth *model.Auth, cloudWatchLogs *cloudwatchlogs.CloudWatchLogs) ([]*cloudwatchlogs.GetQueryResultsOutput, error) {
	...
	updatedEvents, err := commanFunction.GetLogsData(clientAuth, startTime, endTime, logGroupName, `fields @timestamp, eventName| filter eventSource = "ecs.amazonaws.com" and (eventName = "UpdateCluster" or eventName = "UpdateContainerInstance" or eventName = "UpdateService" or eventName = "UpdateTaskSet" or eventName = "RegisterTaskDefinition")| stats count(*) as EventCount by eventName`, cloudWatchLogs)
	if err != nil {
		return nil, err
	}
