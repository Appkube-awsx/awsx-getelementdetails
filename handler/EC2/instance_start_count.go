...
type InstanceStartCountPanelData struct {
	RawData []*cloudwatchlogs.ResultField `json:"InstanceStartCountData"`
}
...
var AwsxEc2InstanceStartCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if authFlag {
			...
			jsonResp, instanceStartCountData, err := GetInstanceStartCountPanel(cmd, clientAuth, nil)
			if err != nil {
				log.Println("Error getting instance start count data: ", err)
				return
			}
			if responseType == "frame" {
				for _, event := range instanceStartCountData.RawData {
					fmt.Println(event)
				}
			} else {
...
func GetInstanceStartCountPanel(cmd *cobra.Command, clientAuth *model.Auth, cloudWatchLogs *cloudwatchlogs.CloudWatchLogs) (string, InstanceStartCountPanelData, error) {
	...
	if startTimeStr != "" {
		...
		if err != nil {
			...
			err := cmd.Help()
			if err != nil {
				return "", InstanceStartCountPanelData{}, err
			}
		}
	...
	instanceStartCountData := InstanceStartCountPanelData{
		RawData: events,
	}

	jsonResp, err := json.Marshal(instanceStartCountData)
	if err != nil {
		log.Println("Error in marshalling JSON: ", err)
		return "", InstanceStartCountPanelData{}, err
	}

	return string(jsonResp), instanceStartCountData, nil
}
